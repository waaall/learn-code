...
<img src="learn-Computer-System.assets/IMG_1822.jpeg" alt="IMG_1822" style="zoom: 33%;" />
...
* ④ 问题：**service这个信号量是干什么用的？正因为这个“消费”号的过程，是需要一个生产者(顾客)和营业员共同完成的，所以这个service表示的是服务员这个资源，应该在服务结束后v**。（上图答案应该有误）
...
![IMG_2464](learn-Computer-System.assets/IMG_2464.jpeg)

> 这个题有问题吧…缺页的前提不应该是先去访问页表吗？那么访问页表不应该是在内核态吗（用户进程不能“知道”页表所在的物理地址）

这是个很好的问题！一定要从硬件实现的角度去考虑，尽可能还原这个过程。



①所谓内核态，在硬件层面就是进程发生了切换(当然，这部分内存地址空间的拥有最好的机器指令权限)。而什么时候会发生这样的切换呢？如何实现的？

在接下来的操作需要操作系统的代码来实现时，这个过程借助中断来实现，准确的说叫“异常”/“内中断”。



②访存过程中，从访问TLB、cache，到页表(通过页表基址寄存器)，虽然是用户进程不可见，但这时的地址空间、CPU上下文等依然是用户进程的，因为这个过程是硬件（MMU）实现的，不需要操作系统的介入。

而假设缺页，一般就需要选择一个现有的页面调出内存，然后将按照该页表项中的硬盘（外存）块号，找到页面放入内存。这个过程涉及到相对复杂（很难仅靠硬件实现）的页面置换等问题，所以就需要触发一个内中断，硬件找到对应的中断处理程序（这里就是操作系统的内存管理部分，页面调度那部分代码），切换上下文，就到了所谓的内核态。



③这一切并不那么死板，假设我们采用的页面置换算法很傻瓜，比如每次都置换最近使用的。即使这样我们还需要加一些硬件，包括存储最近使用页面地址的寄存器，包括通过磁盘块号直接访问磁盘并加载到内存的一系列硬件电路。

这样的话，我们就可以用硬件来实现缺页的解决，而不需要切换到操作系统地址空间。

其实，MMU这部分硬件，本身就是这样的解决方式，用专用的硬件来实现这个过程，不需要“代码”，因为硬件就是代码，而这两者的取舍也是在动态变化的，比如现代CPU有很多专用的计算模块，比如神经网络引擎，甚至苹果在Mac上的T2芯片，负责加密和加速视频解码等，而这些，也本可以完全用操作系统软件实现。



**简单的来说，“正常”访存过程由硬件实现，不需要中断，而发生缺页时需要中断，但是正如系统调用，缺页时仍在用户态(CPU上下文和地址空间都是用户进程的)**
...
![访存过程](learn-Computer-System.assets/访存过程.jpeg)
