...
4. 其实上图整个过程除了页表都在CPU中，而物理地址放在CPU中的MAR寄存器中，右上角的数据，是在MDR寄存器中。而若cache 不命中，就会去访存，就是下面这张图了：



5. 上图右上部分的数据为何与地址最后5位直接相连，什么意思？见下图：
...
<img src="learn-Computer-Organization.assets/访存过程.jpeg" alt="访存过程"  />
...
I/O外设通过总线与CPU通信，方式主要分为三种：程序查询、中断、DMA。
...
中断并不是“专门”针对外设(I/O设备)来实现的，整个中断系统还用来实现异常处理（包括系统调用、指令执行错误、硬件故障等）。



外设某些操作可以引发中断，其处理流程与异常一致，都是由相关硬件来完成，那这个硬件长的什么样？又是怎样的过程呢？见下图：

![中断](learn-Computer-Organization.assets/中断.jpeg)
...
DMA就是在解决很多时候没必要频繁中断的问题 (比如键盘就需要频繁中断来保证相应，但是硬件传输文件，就需要一个较大的缓冲区，而且还需要一部分硬件来执行比如读取多个间断硬盘块的工作)。

**所谓查询方式，正如很多计算机系统的部分，不仅需要计算机本身支持，外设的硬件和驱动程序都需要作出适配，所以这整个机制，是外设如硬盘与芯片厂商、操作系统厂商的合作，共同遵守的“规则”。**
