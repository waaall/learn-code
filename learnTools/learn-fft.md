...
## 1. 采样率 Fs 的作用
### (a) 可检测的最高频率（Nyquist 限制）
- 离散时间信号是由连续信号以 Fs 采样得到的。根据奈奎斯特采样定理：
    - **最高可检测频率 = Fs/2**。
    - 超过一半采样率的频率分量会折叠（aliasing）到低频，无法区分。  
        👉 因此，**Fs 决定了可检测频率的上限（带宽）**。

### (b) 频率分辨率的影响（间接）
- 频率分辨率定义为：
    Δf=FsN=1T
    - 若 **N 固定**，则 Fs 越大，Δf 越粗。
    - 但若 **T 固定**，Δf 与 Fs 无关，只由 T 决定。  
        👉 所以，**分辨率取决于观测时长 T，而不是 Fs 本身**。

### (c) 动态范围 & 相位精度
- Fs 高 → 量化后的采样点更密，信号在时域表现得更“光滑”，相位估计可以更稳定。
- 但频率估计精度的主导因素还是 T，不是 Fs。
...
- 每个 FFT bin 的间隔：
    Δf=1T
- 解释：在 T 秒的观测时间内，信号的最小“可分辨频率差”就是两个正弦在 T 秒内至少差一个整周期。  
    **T 越长，Δf 越小 → 分辨率更高。**


### (b) 可检测的最低频率
- 最低可分辨频率 ≈ Δf=1/T。
- 原理：如果信号频率低于 1/T，在观测窗里甚至都没完整走一个周期，FFT 就无法分辨它和直流的区别。  
- 因此，**T 决定了有效频率的下限**。

### (c) 频率定位精度（区别于分辨率）

- 信号频率估计的精度理论下界（Cramér–Rao Bound）近似为：
    σf2∝1SNR⋅T3
- 解释：在长时间观测中，正弦信号的相位会持续“积累”，相当于放大了频率差，定位精度提升很快。  
    精度远优于 Δf，而且强烈依赖 T 和 SNR。**
    

### (d) 代价：时延

- T 越长 → 分辨率和精度提升，但分析延迟增大。  
    这是实时性 vs 频率精度的根本矛盾。


## 3. 窗函数（Windowing）的作用

FFT 实际上是“信号 × 窗函数”的傅里叶变换。
- 默认情况下是矩形窗（直接截断）。
- 窗函数影响频谱泄漏、主瓣宽度、幅度估计偏差。

### (a) 矩形窗

- 主瓣窄（分辨率高）；
- 旁瓣高（–13 dB），泄漏严重；
- 适合相干采样（信号频率正好落在 FFT bin 上）。

### (b) 加窗（Hann、Blackman 等）
- 频域表现：
    - 主瓣变宽（分辨率下降）；
    - 旁瓣降低（泄漏减少）。
...
- 加窗：幅度被“衰减”，需做修正（每个窗有校正系数）。
    

### (b) 动态范围

- 高旁瓣窗（矩形窗） → 强信号泄漏淹没弱信号，动态范围差。
    
- 低旁瓣窗（Blackman, Flat-top） → 动态范围好，可以区分强/弱信号。
    

### (c) 噪声处理

- 窗函数对噪声的等效带宽不同。宽主瓣窗（如 Flat-top） → 噪声功率增加 → SNR 下降。
    
- 短 T 时，噪声平均效果差；长 T 时，频率估计方差下降。
...
|参数|影响的物理量|原理解释|工程取舍|
|---|---|---|---|
|采样率 Fs|有效频率上限 = Fs/2|Nyquist 限制|选够带宽 + 抗混叠裕量即可|
||分辨率（间接）|Δf = Fs/N，但实质上由 T 决定|若固定 T，Fs 对分辨率无影响|
|观测时长 T|分辨率 Δf = 1/T|频率差需 ≥ 1/T 才能分辨|T 长 → 分辨率高，但延迟大|
||有效频率下限 ≈ 1/T|太低频在观测窗内像直流|T 长才能检测超低频|
||频率精度|CRLB ~ 1/(SNR·T³)|T 长 → 精度提升巨大|
|窗函数|分辨率 vs 泄漏|主瓣宽、旁瓣低|Hann/Blackman 常用|
||幅度估计|需校正因子|平顶窗精度最佳|
||动态范围|旁瓣越低动态范围越好|根据信号特点选择窗|
...
FFT还是有很多问题，比如频率分辨率和精度不足，采样窗口短导致频率泄漏等等。针对这些问题有一系列的优化算法。

## IpDFT(插值)

### 1. 核心问题：频谱泄漏与栅栏效应

要理解IpDFT为什么需要，首先要明白DFT（离散傅里叶变换）的固有局限性：

- **DFT的量化特性**：DFT将一个信号分解为一系列离散的频率点（称为**频率bin**），每个bin的中心频率为 `k * fs / N`，其中 `k` 是整数（bin的索引）。
    
- **栅栏效应（Fence Effect）**：这意味着DFT只能看到这些离散bin上的频谱。如果信号的实际频率 `f_actual` 恰好等于某个bin的中心频率，那么我们就能得到精确的测量结果。但绝大多数情况下，`f_actual` 会落在两个bin之间。
    
- **频谱泄漏（Spectral Leakage）**：当信号频率不在bin中心时，信号的功率会“泄漏”到相邻的多个bin中。这使得：
    
    1. **幅值测量不准**：主瓣（峰值）的幅值被低估了。
        
    2. **频率测量不准**：你只能知道峰值在`k`和`k+1`bin之间，但不知道具体位置。
        

**这就好比用一把只有厘米刻度的尺子去测量一个8.3毫米的物体，你只能读出“8或者9厘米”，但不知道它到底是8毫米还是9毫米。** DFT的bin就像是尺子上的刻度。

### 2. IpDFT的原理：插值“看清”栅栏之间

IpDFT的核心思想就是：**既然真实的峰值落在两个DFT bin之间，我们可以利用峰值附近几个bin的幅值（或实部/虚部）来拟合出真实的峰值位置和高度**，从而“看清”栅栏之间的细节。
使用的是最经典和高效的方法之一：**Jacobsen 方法**（基于三点幅值或实部的抛物线拟合）。



## [窗函数](https://zh.wikipedia.org/zh-sg/窗函数)
因为傅立叶变换的理想条件是连续无限的周期函数，对于一个时间段的采样窗口，时序信号会被“强行被周期型”，但是边界可能就会不连续，导致出现高频噪声(本没有)；窗函数的目的就是让边界过度到0，强行让它连续。没有加窗操作就约等于加了一个矩形窗。

### 窗函数的优缺点&使用场景

1. **振荡检测的目的不同**：在振荡检测中，我们通常关注的是**是否存在明显的振荡频率**，而不是精确测量每个频率分量的准确幅度。窗函数的主要作用是减少频谱泄漏，提高频率分辨率和幅度精度，但这在振荡检测中可能不是最重要的。
    
2. **矩形窗的优势**：
    - **最高的频率分辨率**：矩形窗（不加窗）的主瓣最窄，可以提供最好的频率分辨率
    - **计算简单**：不需要额外的窗函数计算
    - **对瞬态振荡更敏感**：对于突然出现的振荡，矩形窗可能更敏感

3. **振荡检测的特点**：
    - 我们通常寻找的是明显高于噪声水平的峰值
    - 关注的是频率是否存在，而不是精确的幅度值
    - 在许多工业应用中，简单的检测方法更实用

#### 什么时候应该使用窗函数？
1. **需要精确测量幅度**时
2. **信号包含多个接近的频率成分**时（减少频谱泄漏）
3. **信号长度较短**时（减少截断效应）
4. **需要进行精确的频率分析**而不仅仅是检测是否存在振荡时
